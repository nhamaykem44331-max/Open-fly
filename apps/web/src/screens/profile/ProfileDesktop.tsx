...
import { SAVED_PASSENGERS, CHANNELS } from '../../data/mock'
...
export function ProfileDesktop() {
  ...
  const [ch, setCh] = useState<Record<string, boolean>>({ telegram: true, email: true, zalo: false, push: true })
    ...
    <div style={{ background: T.canvas, minHeight: '100%', paddingBottom: 70 }}>
      <Container max={1100} style={{ paddingTop: 48 }}>
        <div style={{ background: T.inkBlock, color: T.onInk, borderRadius: 16, padding: 32, display: 'flex', alignItems: 'center', gap: 24, marginBottom: 28 }}>
          <div style={{ width: 76, height: 76, borderRadius: '50%', background: 'rgba(245,241,234,0.1)', border: '1px solid rgba(245,241,234,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: T.serif, fontSize: 30, color: T.rustSoft, fontStyle: 'italic', fontWeight: 600, flexShrink: 0 }}>An</div>
          <div style={{ flex: 1 }}>
            <h1 style={{ fontFamily: T.serif, fontSize: 32, fontWeight: 400, letterSpacing: '-1px', color: T.onInk, margin: 0 }}>Đào Andy</h1>
            <div style={{ fontFamily: T.sans, fontSize: 13.5, color: 'rgba(245,241,234,0.6)', marginTop: 4 }}>andy.dao@gmail.com · +84 938 121 234</div>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, marginTop: 12, padding: '5px 14px', borderRadius: 100, background: 'rgba(245,241,234,0.08)', border: '1px solid rgba(245,241,234,0.18)', fontFamily: T.sans, fontSize: 12, fontWeight: 600, letterSpacing: 0.5, color: T.rustSoft }}><Ic.spark size={13} stroke={T.rustSoft} />Thành viên Premium</span>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22, alignItems: 'start' }}>
          <div style={{ background: T.paper, border: `1px solid ${T.line}`, borderRadius: 12, padding: 26 }}>
            ...
            {SAVED_PASSENGERS.map((p, i) => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderBottom: i < SAVED_PASSENGERS.length - 1 ? `1px solid ${T.line}` : 'none' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {Object.entries(CHANNELS).map(([id, c]) => (
                <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 10, background: T.paper2 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 8, background: T.paper, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${T.line}` }}><ChannelIcon kind={id} size={17} active={ch[id]} /></div>
                  ...
                  <Toggle on={ch[id]} onClick={() => setCh((s) => ({ ...s, [id]: !s[id] }))} />
                </div>
              ))}
            </div>
          </div>
